...
import { downloadAllImages } from '../../services/download-service';
import { PLATFORM_CONFIGS } from '../../config/constants';

/**
 * 다운로드 섹션 컴포넌트
 * 원본 index.html lines 1124-1137 + CSS lines 470-476 기반
 *
 * - 플랫폼 뱃지 (SmartStore, Coupang, 11번가, G마켓/옥션)
 * - 전체 다운로드 ZIP 버튼
 */
export default function DownloadSection() {
  const { generatedImages, generatedSections, productName } = useApp();
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      const result = await downloadAllImages(generatedImages, generatedSections, productName);
      showToast(
