...
/**
 * 체크리스트 섹션 컴포넌트
 * 원본 index.html lines 1139-1160 + functions at lines 4617-4865 기반
 *
 * - "품질 체크 시작" 버튼
 * - 분석 결과: 스코어 바, 카테고리별 항목, 개선 제안
 */
export default function ChecklistSection() {
  ...
  const handleRunChecklist = useCallback(async () => {
    ...
    productFeatures, additionalNotes,
    generatedSections, generatedImages,
    useBackend, geminiApiKey, showToast,
  ]);
  ...
  const scorePercent = result
    ...
    : 0;
  const scoreClass = scorePercent >= 70
    ? 'bg-accent-success'
    : scorePercent >= 50
      ? 'bg-accent-warning'
      : 'bg-accent-danger';
