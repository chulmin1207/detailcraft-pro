...
/**
 * 헤더 컴포넌트
 * 원본 index.html lines 801-825 (.header, .logo, .api-section) 기반
 *
 * 표시 요소:
 * - 좌측: 로고 마크 + 타이틀 + 버전 뱃지
 * - 우측: 유저 정보 + 로그아웃, 테마 토글, Claude/Gemini API 상태 점, API 설정 버튼
 *
 * @param {Function} onOpenApiSettings - API 설정 모달 열기 콜백
 */
export default function Header({ onOpenApiSettings }) {
  const { currentUser, logout } = useAuth();
  const { theme, toggleTheme } = useTheme();
  const { claudeApiKey, geminiApiKey, useBackend } = useApp();

  // useBackend가 true면 서버를 통해 연결되므로 항상 connected
  const claudeConnected = useBackend || !!claudeApiKey;
  const geminiConnected = useBackend || !!geminiApiKey;
  const claudeLabel = useBackend ? '기획 AI (서버)' : (claudeConnected ? '기획 AI 연결됨' : '기획 AI 미연결');
  const geminiLabel = useBackend ? '이미지 AI (서버)' : (geminiConnected ? '이미지 AI 연결됨' : '이미지 AI 미연결');
