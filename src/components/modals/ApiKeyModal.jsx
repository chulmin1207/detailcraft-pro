...
/**
 * API 키 설정 모달
 * 원본 index.html lines 1164-1209 (HTML), lines 1862-1917 (JS) 기반
 *
 * Claude API Key (기획서 생성용)와 Gemini API Key (이미지 생성용)를
 * 입력/저장/삭제할 수 있는 모달.
 * API 키는 브라우저 localStorage에만 저장된다.
 */
export default function ApiKeyModal({ isOpen, onClose }) {
  const { claudeApiKey, geminiApiKey, saveApiKeys, clearApiKeys } = useApp();
  const { showToast } = useToast();

  // 로컬 입력값 상태 (모달 내부에서만 사용)
  // isOpen이 변할 때 초기값을 리셋하기 위해 key 패턴 대신 직접 추적
