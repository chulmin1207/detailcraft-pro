...
/**
 * 카피 대안 추천 모달
 * 원본 index.html lines 486-502 (CSS), lines 2748-2913 (JS) 기반
 *
 * 헤드라인 또는 서브카피의 대안을 보여주고, 클릭하면 해당 값을 적용한다.
 * "새로 10개 생성" 버튼으로 Gemini API를 호출하여 새 대안을 생성할 수 있다.
 *
 * @param {boolean} isOpen - 모달 표시 여부
 * @param {Function} onClose - 닫기 핸들러
 * @param {number} sectionIndex - 대상 섹션 인덱스
 * @param {'headline' | 'subcopy'} type - 카피 유형
 */
export default function CopySuggestModal({ isOpen, onClose, sectionIndex, type }) {
  const {
    generatedSections,
  ...
  // Gemini API를 호출하여 새 대안 10개 생성
  // 원본 regenerateCopyWithCredit (lines 2797-2895) 기반
...
["첫번째", "두번째", "세번째", "네번째", "다섯번째", "여섯번째", "일곱번째", "여덟번째", "아홉번째", "열번째"]`;
      ...
      // 섹션 데이터에 저장 (다음에 다시 열면 저장된 것 표시)
      setGeneratedSections((prev) => {
        const updated = [...prev];
        if (type === 'headline') {
          updated[sectionIndex] = { ...updated[sectionIndex], headlineAlts: newSuggestions };
        } else {
          updated[sectionIndex] = { ...updated[sectionIndex], subCopyAlts: newSuggestions };
        }
        return updated;
      });
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`${typeLabel} 대안`}>
      ...
      <div className="max-h-[400px] overflow-y-auto -mx-6 px-3">
        {loading ? (
          ...
          suggestions.map((copy, i) => (
            ...
            >
              <p className="text-[0.95rem] leading-relaxed text-text-primary">{copy}</p>
            </div>
          ))
        ) : (
          // 비어있는 상태
      ...
      <div className="mt-4 pt-4 border-t border-border-subtle text-center">
        <p className="text-[0.8rem] text-text-tertiary mb-2.5">마음에 드는 게 없나요?</p>
        <button
