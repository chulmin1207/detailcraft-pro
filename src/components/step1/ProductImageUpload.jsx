...
export default function ProductImageUpload() {
  const { uploadedImages, setUploadedImages, refStrength, setRefStrength } = useApp();

  // 이미지 추가 핸들러 팩토리
  const handleAdd = useCallback((type) => (newImages) => {
      ...
      <div className="p-6">
        <div className="grid grid-cols-1 gap-[18px]">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            {/* 레퍼런스 반영 강도 선택 - 레퍼런스 이미지 업로드 시에만 표시 */}
            {hasReferences && (
              <div className="mt-3">
                ...
                <div className="flex gap-3 mt-2">
                  ...
                  >
                    <input
                      ...
                      name="refStrength"
                      value="weak"
                      checked={refStrength === 'weak'}
                      onChange={() => setRefStrength('weak')}
                      className="w-[18px] h-[18px] accent-accent-gemini cursor-pointer"
                    />
                    <span className="flex flex-col gap-0.5">
                      <strong className={`text-[0.9rem] ${refStrength === 'weak' ? 'text-accent-gemini' : 'text-text-primary'}`}>
                        약하게
                      </strong>
                  ...
                  >
                    <input
                      ...
                      onChange={() => setRefStrength('strong')}
                      className="w-[18px] h-[18px] accent-accent-gemini cursor-pointer"
                    />
                    <span className="flex flex-col gap-0.5">
