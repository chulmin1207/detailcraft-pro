...
/**
 * 개별 섹션 편집기 컴포넌트 (아코디언)
 * 원본 index.html renderSectionsEditor (lines 2510-2600) 기반
 *
 * @param {Object} section - 섹션 데이터 (number, name, purpose, headline, subCopy, visualPrompt, headlineAlts, subCopyAlts)
...
export default function SectionEditor({ section, index }) {
  ...
  const [sectionModel, setSectionModel] = useState(selectedModel);
  const [isGenerating, setIsGenerating] = useState(false);
  const [showSuggestModal, setShowSuggestModal] = useState(false);
  const [suggestType, setSuggestType] = useState('headline');

  const hasImage = generatedImages[index] && !generatedImages[index].error;
  const sectionRefs = sectionReferences[index] || [];

  // 섹션 데이터 업데이트 헬퍼
  const updateSection = useCallback((field, value) => {
  ...
  // 섹션별 레퍼런스 이미지 추가
  const handleAddRef = useCallback((newImages) => {
  ...
  // 섹션별 레퍼런스 이미지 제거
  const handleRemoveRef = useCallback((imgIndex) => {
    setSectionReferences((prev) => {
      const current = [...(prev[index] || [])];
  ...
  // AI 카피 추천 모달 열기
  const handleSuggestCopy = useCallback((type) => {
    setSuggestType(type);
    setShowSuggestModal(true);
  }, []);

  // 대안 카피 적용
  const handleApplySuggestion = useCallback((type, value) => {
    if (type === 'headline') {
  ...
  const handleGenerateImage = useCallback(async () => {
    ...
    try {
      ...
      setGeneratedImages((prev) => ({
        ...prev,
        [index]: { data: imageData.dataUrl, base64: imageData.base64, prompt: imageData.prompt },
      }));
      ...
      showToast(`섹션 ${section.number} 실패: ${err.message}`, 'error');
    } finally {
      setIsGenerating(false);
    }
  }, [
    useBackend, geminiApiKey, sectionModel, section, index,
    uploadedImages, sectionReferences, selectedAspectRatio,
    productName, category, productFeatures, additionalNotes,
    generatedSections, refStrength, targetAudience,
    setGeneratedImages, showToast,
  ]);

  // 대안 카피 목록
  const alts = suggestType === 'headline' ? section.headlineAlts : section.subCopyAlts;
            ...
            <div className="mb-4">
              ...
              <div className="flex gap-2 items-center">
                ...
                <button
                  ...
                  "
                  style={{ background: 'linear-gradient(135deg, var(--accent-primary), var(--accent-gemini))' }}
                >
                  &#x2728;
                </button>
              </div>
