...
export function compressImageForAPI(base64, maxWidth = 800, quality = 0.5) {
    return new Promise((resolve) => {
        const img = new Image();
        img.onload = () => {
            const canvas = document.createElement('canvas');
...
// 이미지 리사이즈 함수
export function resizeImage(base64, targetWidth) {
    return new Promise((resolve) => {
        const img = new Image();
        img.onload = () => {
            const canvas = document.createElement('canvas');
 ...
 * @param {Object} section - 섹션 정보 (number, name, headline, subCopy, purpose, visualPrompt)
 * @param {number} index - 섹션 인덱스
 * @param {Object} options - 추가 옵션
 * @param {string} options.productName - 제품명
 * @param {string} options.category - 카테고리
 * @param {string} options.productFeatures - 제품 특징
 * @param {string} options.additionalNotes - 추가 요청사항
 * @param {Object} options.uploadedImages - 업로드된 이미지 ({ product, package, references })
 * @param {Object} options.sectionReferences - 섹션별 레퍼런스 이미지
 * @param {string} options.refStrength - 레퍼런스 강도 ('strong' | 'light')
 * @param {Array} options.generatedSections - 생성된 섹션 배열
 * @param {string} options.selectedAspectRatio - 선택된 비율
 * @param {string} options.headline - 헤드라인 (입력값 우선)
 * @param {string} options.subCopy - 서브카피 (입력값 우선)
 * @param {string} options.userVisualPrompt - 사용자 비주얼 프롬프트
 * @param {string} options.targetAudience - 타겟 오디언스
 * @returns {string} 완성된 프롬프트 문자열
 */
export function buildImagePrompt(section, index, options = {}) {
    const {
        productName = '',
        category = '',
        productFeatures = '',
 ...
 *
 * @param {Object} params
 * @param {Object} params.section - 섹션 정보
 * @param {number} params.index - 섹션 인덱스
 * @param {Object} params.modelConfig - 모델 설정 ({ model, config, timeout, name })
 * @param {Object} params.uploadedImages - 업로드된 이미지 ({ product, package, references })
 * @param {Object} params.sectionReferences - 섹션별 레퍼런스 이미지
 * @param {Object} [params.step3Options] - STEP 3 옵션 (null이면 기본 생성)
 * @param {boolean} [params.step3Options.includeGenerated] - 기존 생성 이미지 포함 여부
 * @param {boolean} [params.step3Options.includeStep1Ref] - STEP 1 전체 레퍼런스 포함 여부
 * @param {boolean} [params.step3Options.includeReference] - 추가 레퍼런스 포함 여부
 * @param {boolean} [params.step3Options.includePrompt] - 추가 프롬프트 포함 여부
 * @param {Array} [params.step3Options.step3Refs] - STEP 3 레퍼런스 이미지 배열
 * @param {string} [params.step3Options.step3Prompt] - STEP 3 사용자 프롬프트
 * @param {Object} [params.step3Options.generatedImages] - 기존 생성된 이미지 맵
 * @param {boolean} params.useBackend - 백엔드 사용 여부
 * @param {string} params.backendUrl - 백엔드 URL
 * @param {string} params.geminiApiKey - Gemini API 키
 * @param {string} [params.authToken] - 인증 토큰
 * @param {string} params.selectedAspectRatio - 선택된 비율
 * @param {string} params.productName - 제품명
 * @param {string} params.category - 카테고리
 * @param {string} params.productFeatures - 제품 특징
 * @param {string} params.additionalNotes - 추가 요청사항
 * @param {Array} params.generatedSections - 생성된 섹션 배열
 * @param {string} params.refStrength - 레퍼런스 강도
 * @param {string} [params.headline] - 헤드라인 (입력값 우선)
 * @param {string} [params.subCopy] - 서브카피 (입력값 우선)
 * @param {string} [params.userVisualPrompt] - 사용자 비주얼 프롬프트
 * @param {string} [params.targetAudience] - 타겟 오디언스
 * @returns {Promise<{dataUrl: string, base64: string, prompt: string}>}
 */
export async function generateSectionImage({
    section, index, modelConfig,
    uploadedImages, sectionReferences, step3Options,
    useBackend, backendUrl, geminiApiKey,
    selectedAspectRatio, productName, category, productFeatures,
    additionalNotes, generatedSections, refStrength,
    headline, subCopy, userVisualPrompt, targetAudience
}) {
    let prompt;
    const parts = [];
