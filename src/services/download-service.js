...
import { resizeImage, base64ToBlob } from './image-service.js';
...
// ===== 전체 이미지 다운로드 (ZIP) =====
export async function downloadAllImages(generatedImages, generatedSections, productName) {
    const images = Object.entries(generatedImages)
        .filter(([, img]) => img && !img.error)
        .map(([i, img]) => ({
            name: `section-${generatedSections[i].number}-${generatedSections[i].name.replace(/[^a-zA-Z0-9가-힣]/g, '_')}.png`,
            data: img.data
        }));
...
// ===== 단일 이미지 다운로드 =====
export function downloadSingle(index, generatedImages, generatedSections) {
