...
import { compressImageForAPI } from './image-service.js';
...
// ===== 기획서 프롬프트 빌더 =====
// data: { productName, category, priceRange, targetAudience, productFeatures, additionalNotes }
// imageAnalysis: AI Vision 분석 결과 객체 또는 null
export function buildPlanPrompt(data, imageAnalysis = null) {
    const d = data;
...
// images: { product: string[], package: string[] } - base64 이미지 데이터 배열
// config: { useBackend, backendUrl, geminiApiKey, authToken }
export async function analyzeProductImages(images, config) {
