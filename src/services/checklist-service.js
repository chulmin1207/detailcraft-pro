...
import { CATEGORY_CHECKLISTS, COMMON_CHECKLIST } from '../config/category-data.js';
// fetchWithTimeout, compressImageForAPI - 직접 사용하지 않으므로 제거

// ===== IMAGE ANALYSIS WITH GEMINI VISION =====
export async function analyzeGeneratedImages(generatedImages, { useBackend, backendUrl, geminiApiKey }) {
...
export function collectAnalysisData({ productName, category, priceRange, productFeatures, generatedSections, generatedImages }) {
    ...
    const hasReviewSection = generatedSections.some(s =>
        (s.name || '').includes('후기') || (s.name || '').includes('리뷰') ||
        (s.headline || '').includes('후기') || (s.headline || '').includes('리뷰')
    );

    // 카테고리별 체크
    const categoryKeywords = {
        snack: { nutrition: ['칼로리', 'kcal', '단백질', '탄수화물'], taste: ['맛', '달콤', '짭짤', '고소'], texture: ['바삭', '쫀득', '부드러운', '크런치'], certification: ['HACCP', '인증', 'ISO'] },
        beverage: { nutrition: ['칼로리', '당류'], storage: ['냉장', '보관'], caffeine: ['카페인', 'mg'] },
        health: { dosage: ['1일', '섭취', '복용'], certification: ['건강기능식품', '식약처'], caution: ['주의', '임산부'] },
        beauty: { usage: ['사용법', '바르', '도포'], skintype: ['지성', '건성', '복합성', '민감성'], certification: ['식약처'] },
        instant: { expiry: ['유통기한', '소비기한'], storage: ['냉장', '냉동', '실온'], cooking: ['조리', '데워', '끓여'] }
    };
...
// ===== 체크리스트 결과 생성 =====
export function generateChecklistResult(data) {
    ...
    const categoryResult = { name: `${categoryConfig.icon} ${categoryConfig.name} 필수정보`, results: [] };
    ...
    if (imgAnalysis.analyzed) {
        const imageQualityResult = { name: '🎨 이미지 품질 (AI 분석)', icon: '🎨', results: [] };

        imageQualityResult.results.push({
            label: '텍스트 가독성',
            detail: imgAnalysis.textQuality === 'good' ? '좋음' : imgAnalysis.textQuality === 'medium' ? '보통' : '개선 필요',
            passed: imgAnalysis.textQuality === 'good' || imgAnalysis.textQuality === 'medium'
        });

        imageQualityResult.results.push({
            label: '디자인 품질',
            detail: imgAnalysis.designQuality === 'good' ? '좋음' : imgAnalysis.designQuality === 'medium' ? '보통' : '개선 필요',
...
// ===== 개선 제안 =====
export function getSuggestion(itemId, data) {
    const suggestions = {
...
// ===== 카테고리별 개선 제안 =====
export function getCategorySuggestion(itemId, label, category) {
