...
// 가격 전략 메시지 조회 헬퍼
export function getPriceMessage(priceRange) {
...
export const COMMON_CHECKLIST = {
    copy: {
        ...
        icon: '📝',
        items: [
            { id: 'headline_length', label: '헤드라인 적정 길이', detail: '15자 이내 권장', check: (data) => data.headlineAvgLength <= 15 },
            { id: 'headline_impact', label: '헤드라인 임팩트', detail: '관심을 끄는 문구', check: (data) => data.hasImpactWords },
            { id: 'cta_exists', label: 'CTA 문구 존재', detail: '구매 유도 문구', check: (data) => data.hasCTA },
            { id: 'product_name', label: '제품명 명확', detail: '제품명이 잘 드러남', check: (data) => data.productNameClear }
        ]
    },
    visual: {
        name: '비주얼',
        icon: '🖼️',
        items: [
            { id: 'product_image', label: '제품 이미지 포함', detail: '실제 제품 사진', check: (data) => data.hasProductImage },
            { id: 'section_count', label: '충분한 섹션 수', detail: '6개 이상 권장', check: (data) => data.sectionCount >= 6 },
            { id: 'package_image', label: '패키지 이미지', detail: '포장 디자인 노출', check: (data) => data.hasPackageImage }
        ]
    },
    trust: {
        name: '신뢰 요소',
        icon: '🛡️',
        items: [
            { id: 'review_section', label: '리뷰/후기 섹션', detail: '고객 후기 영역', check: (data) => data.hasReviewSection },
            { id: 'price_info', label: '가격 정보', detail: '가격 명시 여부', check: (data) => data.hasPriceInfo }
        ]
    }
};
